...
`case_match()` or `case_when()` (or simply `recode()`) allow us to label our numeric data with more human-readable descriptions. `if_else()` is a useful shorthand for cases where we only have two categories.
