...
-   `a`, `multi` etc. are the variable names [(some naming rules, e.g., no whitespace, must not start with a number, many special characters not allowed)]{style="font-size: 16px;"}
...
-   Select "*New Directory*" [(if you already have a folder for this course, you can choose "*Existing directory*" and select that folder)]{style="font-size: 16px"} and then choose "*New Project*" at the top of the list.
...
[PS: `R` can deal with folder and file names that contain spaces, but since some programms can't, it's best practice not to use whitespaces for file/folder naming.]{style="font-size: 16px"}
